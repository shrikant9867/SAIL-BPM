...
def get_delivery_note_data(doc):
	#'{:.3f}'.format(round(2606.89579999999, 3))
	#print("#########@@@@@@@@@@@@@@@@@@")
	...
	#print("kkkkkkkkkkkkkkkkk",data_aggregate)
	table_height=0
	sendata={}
	total_bundle=0
	total_weight=0
	...
	for i in data_aggregate:
		...
		#header = """ <table style ="width :350px"><tbody><tr class="cls_003" style="border: 1px solid black;"><th colspan ="4"  style="text-align:center" ><strong >%s</strong></th></tr><tr ><td><strong>NO</strong></td><td><strong>Wt Range</strong></td><td><strong>Qty</strong></td><td><strong>Gross Wt</strong></td></tr>"""%(i[0])
		...
		for j in data:
			if j[0] == i[0]:
				...
				table_height += 1
				count+=1
		#header += """<tr><td><strong>%s</strong></td><td><strong>%s</strong></td><td><strong>%s</strong></td><td><strong>%s</strong></td></tr></tbody></table><p   align="justify">&nbsp;</p>"""%(count,"Bun",i[2],i[1])
